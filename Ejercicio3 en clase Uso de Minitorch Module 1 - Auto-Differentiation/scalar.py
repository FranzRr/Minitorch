...
def central_difference(f, *vals, arg=0, epsilon=1e-6):
    ...
    for i in range(len(vals)):
        if i == arg:
            lista_1.append(vals[i] + epsilon)
            lista_2.append(vals[i] - epsilon)
        else:
            lista_1.append(vals[i])
            lista_2.append(vals[i])
        
    expresion = (f(*lista_1) - f(*lista_2))/(2*epsilon)
    return expresion
...
class ScalarFunction(FunctionBase):
    ...
    def backward(ctx, d_out):
        ...
        for i in range(len(inputs)):
            respuesta.append(central_difference(FunctionBase, *inputs, arg = i)*d_out)
