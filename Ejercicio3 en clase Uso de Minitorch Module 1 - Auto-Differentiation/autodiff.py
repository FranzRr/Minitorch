    ...
    def chain_rule(cls, ctx, inputs, d_output):
        ...
        for i in range(len(inputs)):
            if is_constant(inputs[i]):
                ...
                var_deriv.append(VariableWithDeriv(inputs[i], deriv[i]))
        return var_deriv
...
def backpropagate(final_variable_with_deriv):
    ...
    while len(queue) > 0:
        Variable_d = queue.pop(0)
        if is_leaf(Variable_d.variable):
            Variable_d.variable._add_deriv(Variable_d.deriv)
        else:
            ctx = Variable_d.variable.history.ctx
            inputs = Variable_d.variable.history.inputs
            d_output = Variable_d.deriv
            v_d = Variable_d.variable.history.last_fn().chain_rule(
                ctx = ctx, 
                inputs = inputs,
                d_output = d_output,
            )
